...
if(isset($_GET['action']) && $_GET['action']==1){
		...
		$arquivo = $Images->Upload('../images/','img',0);

		if(!empty($arquivo)){
			$sel = $db->select("INSERT INTO pop_up (urli, cidade) VALUES ('$arquivo','$cidade')");
...
//FAZ A PESQUISA PARA ALTERAÇAO NO BANCO DE DADOS	
	...
	$sel = $db->select("SELECT * FROM pop_up WHERE id='$id' LIMIT 1");
	$ln = $db->expand($sel);
...
//FAZ A exclusao NO BANCO DE DADOS		
	...
	$sel = $db->select("DELETE FROM pop_up WHERE id='$id' LIMIT 1");
	echo '<script>$("#sucesso_user").show();</script>';
...
//FAZ A ALTERAÇAO NO BANCO DE DADOS		
		...
		if(!empty($arquivo)){
			$sel = $db->select("UPDATE pop_up SET urli='$arquivo' WHERE id='$id' LIMIT 1");	
			echo '<script>$("#sucesso_user").show();</script>';
		}

		$sel = $db->select("UPDATE pop_up SET cidade='$cidade' WHERE id='$id' LIMIT 1");	
			echo '<script>$("#sucesso_user").show();</script>';
...
<div class="panel panel-primary">
	...
	<div class="panel-heading">
		<h4 class="panel-title"><h4>Cadastro de Pop-Up</h4></h4>
	</div>
    ...
    <div class="col-md-12">
    	...
    	<div class="col-md-6">
           <div class="form-group">
                <label for="exampleInputEmail1">Localidade a ser mostrado</label>
        ...
        <div class="col-md-6">
           <div class="form-group">
                <label for="exampleInputEmail1">POP-UP <small>(max: 2 MB)</small><span style="color: red;"> **</span>  </label>
...
<div class="row">
	...
	<?php
		$hoje=date("Y-m-d");
		$db = new DB();    
		$sel = $db->select("SELECT * FROM pop_up ");
		...
		if($db->rows($sel)){
			...
			while($yy = $db->expand($sel)){
				...
				echo'
						...
						<img src="../images/'.$yy['urli'].'" class="img-responsive" ><br>
